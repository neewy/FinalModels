...
public class ADD_CAR{
	...
	/*@ public normal_behavior
		requires true;
 		assignable \nothing;
		ensures \result <==> (machine.OBJECTS.difference(machine.get_objects()).has(Car) && NAT.instance.has(Desc) && NAT.instance.has(W) && NAT.instance.has(H) && INT.instance.has(X) && INT.instance.has(Y) &&  (\forall Integer CarExisted;((machine.get_objects().has(CarExisted)) ==> ((new Integer(machine.get_posX().apply(CarExisted) - X)).compareTo(new Integer(new Integer(machine.get_width().apply(CarExisted) / new Integer(2)) + new Integer(W / new Integer(2)))) > 0 || (new Integer(machine.get_posY().apply(CarExisted) - Y)).compareTo(new Integer(new Integer(machine.get_height().apply(CarExisted) / new Integer(2)) + new Integer(H / new Integer(2)))) > 0 || (new Integer(X - machine.get_posX().apply(CarExisted))).compareTo(new Integer(new Integer(machine.get_width().apply(CarExisted) / new Integer(2)) + new Integer(W / new Integer(2)))) > 0 || (new Integer(Y - machine.get_posY().apply(CarExisted))).compareTo(new Integer(new Integer(machine.get_height().apply(CarExisted) / new Integer(2)) + new Integer(H / new Integer(2)))) > 0))) && NAT.instance.has(M) && NAT.instance.has(F)); */
